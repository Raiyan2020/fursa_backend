@php
    ...
    $selectedInterests = collect(old('interest_ids', optional($opportunity)->masterInterests?->pluck('id')->all() ?? []))->map(fn ($id) => (string) $id);
...
<div class="row">
    ...
    <div class="col-md-3 mb-1">
        <label>{{ __('learning type') }}</label>
        <select name="learning_type_id" class="form-control{{ $invalid('learning_type_id') }}">
            <option value="">{{ __('select') }}</option>
            @foreach ($learningTypes as $type)
                <option value="{{ $type->id }}" {{ $selected('learning_type_id') === (string) $type->id ? 'selected' : '' }}>
                    {{ $choiceLabel($type) }}
                </option>
            @endforeach
        </select>
        @error('learning_type_id') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
    </div>
    <div class="col-md-3 mb-1">
        <label>{{ __('format') }}</label>
        <select name="format_id" class="form-control{{ $invalid('format_id') }}">
            <option value="">{{ __('select') }}</option>
            @foreach ($formats as $type)
                <option value="{{ $type->id }}" {{ $selected('format_id') === (string) $type->id ? 'selected' : '' }}>
                    {{ $choiceLabel($type) }}
                </option>
            @endforeach
        </select>
        @error('format_id') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
    </div>
    <div class="col-md-3 mb-1">
        <label>{{ __('certificate type') }}</label>
        <select name="certificate_type_id" class="form-control{{ $invalid('certificate_type_id') }}">
            <option value="">{{ __('select') }}</option>
            @foreach ($certificateTypes as $type)
                <option value="{{ $type->id }}" {{ $selected('certificate_type_id') === (string) $type->id ? 'selected' : '' }}>
                    {{ $choiceLabel($type) }}
                </option>
            @endforeach
        </select>
        @error('certificate_type_id') <div class="invalid-feedback d-block">{{ $message }}</div> @enderror
    </div>
