# Fursa — رفع صورة الحساب/الفرصة (POST) + إجراءات جديدة على الفرصة
...
"عدّل وأعد الإرسال" — الفرونت يستخدم `POST /api/volunteer-opportunities/{id}/`
(قسم 3 تحت) الأول عشان المستخدم يعدّل البيانات (لو محتاج)، وبعدين ينادي على
...
## 3) تعديل الفرصة وصورها — بقوا `POST` بس (نفس سبب قسم 1)

نفس مشكلة رفع الصور بتاعة الحساب كانت موجودة هنا بالظبط، فاتصلحت بنفس
الطريقة — المسارين دول بقوا `POST` بدل `PATCH`/`PUT`:

```
POST /api/volunteer-opportunities/{id}/               ← تعديل الفرصة (بيقبل صور)
POST /api/volunteer-opportunities/{id}/update_images/  ← مخصص لتحديث الصور بس
```

`PATCH`/`PUT` القديمين اتشالوا خالص من الراوت — أي طلب لسه بيتبعت بيهم
هيرجّع **405 Method Not Allowed**. غيّر الـ method في الفرونت لـ `POST` بس،
باقي شكل الـ body والاستجابة زي ما هو تمامًا (نفس الحقول القديمة، بما فيها
`interest_ids` و`new_opportunity_images_*` لصور الإعلان الجديدة).
...
- خطأ 405 على `/api/account/` أو `/api/volunteer-opportunities/{id}/` أو
  `/api/volunteer-opportunities/{id}/update_images/` معناه إن حد لسه بيبعت
  PATCH/PUT بدل POST على المسارات دي — الاتنين اتشالوا خالص.
